...
void traffic_graph::_process_short(const unordered_set<size_t> & _short_index,
	...
	for(size_t i = 0; i < __short_data.n_cols; i ++ ) {
		...
		for (const auto & ref: res_mp) {
			printf("[Short Edge Overall Loss: %lf]: Clustering Loss: %lf,\
					Clustering Size: %ld, Aggregation size: %ld, Time Range: %lf.\n",
				   ref.second,
