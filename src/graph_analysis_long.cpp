...
void traffic_graph::_process_long(const unordered_set<size_t> & _long_idx,
								  const arma::mat & centroids_long,
								  const arma::Row<size_t> & assign_long) {
	...
	for (size_t i = 0; i < assign_long.size(); i ++) {
		const auto v		= assign_long[i];
		const auto _range	= p_long_edge->at(idx_long_remap[i])->get_time_range();

		if (__long_cluster_size.count(v)) {
			__long_cluster_size[v] ++;
			__long_cluster_idx[v].push_back(idx_long_remap[i]);

			__long_cluster_str[v] = min(__long_cluster_str[v], _range.first);
			__long_cluster_stp[v] = max(__long_cluster_stp[v], _range.second);
		} else {
			__long_cluster_size.insert({v, 1});
			__long_cluster_idx.insert({v, {idx_long_remap[i]}});

			__long_cluster_str.insert({v, _range.first});
			__long_cluster_stp.insert({v, _range.second});
		}
	}

	vector<size_t>					long_origin_index;
	vector<size_t>					long_cluster_size;
	...
	for (size_t i = 0; i < assign_long.size(); i ++ ) {
		if (assign_long[i] == SIZE_MAX) {
			continue;
		}

		if (!has_counted_long.count(assign_long[i])) {
			long_cluster_size.push_back(__long_cluster_size[assign_long[i]]);
			long_origin_index.push_back(idx_long_remap[i]);

			const auto pe	= p_long_edge->at(idx_long_remap[i]);
			...
			has_counted_long.insert(assign_long[i]);
			long_cluster_time.push_back(
					__long_cluster_stp[assign_long[i]] - __long_cluster_str[assign_long[i]]);
			long_origin_index_vec.push_back(__long_cluster_idx[assign_long[i]]);
		}
	}
	...
	if (opt.check() == sat) {
		...
		vector<bool> is_add(addr_idx_mp.size(), false);
		for (const auto & ref: addr_long_lst) {
			if (true) {
				vector<size_t> _conn_edge;
				for (size_t i = 0; i < addr_idx_mp.size(); i ++) {
					if (!is_add[i] &&
						(addr_idx_mp[i].first == ref.second ||
						 addr_idx_mp[i].second == ref.second)) {
	...
	for (const auto & ref : selected_long_daddr) {
		...
		for (size_t i = 0; i < ref.second.size(); i ++) {
			_loss_long_vec.push_back(
				...
				+ log2(long_cluster_size[ref.second[i]] + 1) * bl
		...
		for (size_t i = 0; i < ref.second.size(); i ++) {
			...
			for (size_t i = 0; i < long_res_mp.size(); i ++) {
				...
				printf("[Long Edge Overal Loss: %6.4lf]: Clustering Loss: %6.4lf, \
						Size: %6ld, Time: %4.2lf.\n",
					   _loss_long_vec[index],
					   _cluster_loss_2[index],
					   long_cluster_size[ref.second[index]],
