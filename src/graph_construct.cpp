...
void traffic_graph::dump_graph_statistic_short(void) const {
	...
	size_t mx_degree_in = 0;
	size_t mx_degree_out = 0;

	for (const auto & ref: vertex_set_short) {
		size_t __ctr_in = 0, __ctr_out = 0;
		__ctr_in += short_edge_in.find(ref) !=
				short_edge_in.end() ? short_edge_in.at(ref).size() : 0;
		__ctr_in += short_edge_in_agg.find(ref) !=
				short_edge_in_agg.end() ? short_edge_in_agg.at(ref).size() : 0;

		__ctr_out += short_edge_out.find(ref) !=
				short_edge_out.end() ? short_edge_out.at(ref).size() : 0;
		__ctr_out += short_edge_out_agg.find(ref) !=
				short_edge_out_agg.end() ? short_edge_out_agg.at(ref).size() : 0;
