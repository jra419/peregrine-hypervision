#include "edge_define_new.hpp"

using namespace hypervision;

const auto __get4_tuple = [] (shared_ptr<basic_packet4> pf, string & saddr,
							  string & daddr, pkt_port_t & sp, pkt_port_t & dp) -> void {
	const auto p4 = dynamic_pointer_cast<basic_packet4>(pf);
...
void long_edge::show_edge(void) const {
	double_t start_tm = p_flow->ts_start, end_tm = p_flow->ts_end;
	string saddr, daddr;
	pkt_port_t sp, dp;
	size_t flow_len = p_flow->len;
...
auto long_edge::get_src_str() const -> string {
	const auto pf4 = dynamic_pointer_cast<basic_packet4>(p_flow);
...
// Calculation based just on the max and min ts.
auto long_edge::get_avg_packet_rate() const -> bool {
	return (double_t)(p_flow->ts_end - p_flow->ts_start) / p_flow->cnt;
}

auto long_edge::is_pulse_flow() const -> bool {
	if (get_avg_packet_rate() > pulse_flow_time_line || p_flow->cnt < pulse_flow_ctr_line) {
		return true;
	} else {
		return false;
	}
...
void short_edge::show_edge(size_t max_show) const {
	...
	const size_t seq_len = p_flow->at(0)->len;

	u_int32_t num_len = 0;

	string str_saddr = "---", str_daddr = "---", str_sp = "-", str_dp = "-";
	string str_agg = __get_agg_str(agg_idx);
	// string str_type = __get_type_str(p_flow->at(0));
	...
	printf("[	%15s:%-6s -> %15s:%-6s ] => Agg Type: %s.\n", str_saddr.c_str(), str_sp.c_str(),
			str_daddr.c_str(), str_dp.c_str(), str_agg.c_str());
...
auto short_edge::get_time_range(void) const -> pair<pkt_ts_t, pkt_ts_t> {
	pkt_ts_t str = numeric_limits<pkt_ts_t>::max();
	pkt_ts_t end = numeric_limits<pkt_ts_t>::min();
	for (const auto pf: *p_flow) {
		str = min(str, pf->ts_start);
...
auto short_edge::get_src_str(void) const -> string {
	...
	const auto pf4 = dynamic_pointer_cast<basic_packet4>(p_f);
	return get_str_addr(tuple_get_src_addr(pf4->flow_id));
}
