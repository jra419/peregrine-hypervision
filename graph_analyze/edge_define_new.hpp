...
class long_edge {
private:
	shared_ptr<basic_packet4> p_flow;
...
class short_edge {
private:
	shared_ptr<vector<shared_ptr<basic_packet4>>> p_flow;
...
public:
	...
	// Not an actual average, here we calculate based just on the max and min ts.
	auto get_avg_interval(void) const -> double_t {
		const auto p_f = p_flow->at(0);
		return (double_t)(p_f->ts_end - p_f->ts_start) / p_f->cnt;
	}
	...
	auto get_pkt_len(void) const -> size_t {
		const auto p_f = p_flow->at(0);
		return p_f->len;
	}

	auto get_pkt_code(void) const -> pkt_code_t {
		const auto p_f = p_flow->at(0);
		return p_f->tp;
	}
