...
void explicit_flow_constructor::construct_flow(size_t multiplex) {
    ...
    const u_int32_t part_size = ceil(((double) parse_result.size()) / ((double) multiplex));
    vector<pair<size_t, size_t> > _assign;
    for (size_t core = 0, idx = 0; core < multiplex; ++ core, idx += part_size) {
        _assign.push_back({idx, min(idx + part_size, parse_result.size())});
    }
    ...
    LOGF("Number of flows: %ld [%ld IPv4].",
            p_construct_result4->size(), p_construct_result4->size());
    flow_double_check();
...
void explicit_flow_constructor::dump_flow_statistic(void) const {
    ...
    LOGF("Constructed IPv4 flow: %8ld", p_construct_result4->size());
...
#define FLOW_BYTE_CHECK
    ...
    for (const auto p_rep: *p_construct_result4) {
        ...
        byte_vec.push_back(
            accumulate(p_seq->begin(), p_seq->end(), 0,
                       [] (size_t a, const decltype(p_seq->at(0)) & b) {
