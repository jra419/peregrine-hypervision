...
def get_result_from_file(addr: str) -> Tuple[List[int], List[float]]:
    dir_list    = os.listdir(addr)
    label       = []
    score       = []

    for file in dir_list:
        df = pd.read_csv(addr + '/' + file)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--dataset', type=str, help='Target for analysis.')
    ...
    # analyze_result(*get_result_from_file(f"data/{args.dataset}/{args.target}/{args.target}.csv"))
    analyze_result(*get_result_from_file(f"data/{args.dataset}/{args.target}"))
